...
为避免把不同问题混在一张图里，本节按“价格弹性、成本与距离、边界重跑、时序响应”四组展示结果。前两组对应固定调度后处理，后两组对应改变可行域的全年重跑。`processed/` 下输出统一口径的汇总表：金额统一换算为百万元，碳代理统一换算为 ktCO2e，边界场景补出相对基准的 delta 和百分比变化，逐时台账也生成了 compact 版本，便于后续继续做二次处理。

#### 价格弹性

单位价格步长先给出年度收入斜率，重定价曲线再展示三类价格扰动下的收入与毛收益变化。

![单位价格步长对应的年度收入斜率](5.7混合最优策略短测试与年度运营分析/results/minimal_sensitivity_5_6/figures/c5_56_price_slope_bar.png)

![价格重定价收入与毛收益对比](5.7混合最优策略短测试与年度运营分析/results/minimal_sensitivity_5_6/figures/c5_56_price_sensitivity_line.png)

#### 成本与距离

生命周期台账重算先看净现金变化，离岸距离曲线再看年化投资负担和项目净现金的联动。

![生命周期重算下的净现金变化](5.7混合最优策略短测试与年度运营分析/results/minimal_sensitivity_5_6/figures/c5_56_cost_delta_bar.png)

![离岸距离 CAPEX 代理敏感性](5.7混合最优策略短测试与年度运营分析/results/minimal_sensitivity_5_6/figures/c5_56_distance_capex_line.png)

#### 边界重跑

边界重跑部分把关键 KPI 和回退小时数分开看，便于判断是收益变化还是可行域收缩在起主导作用。

![边界场景关键指标网格](5.7混合最优策略短测试与年度运营分析/results/minimal_sensitivity_5_6/figures/c5_56_boundary_kpi_line_grid.png)

![边界场景回退小时数对比](5.7混合最优策略短测试与年度运营分析/results/minimal_sensitivity_5_6/figures/c5_56_boundary_relaxation_bar.png)

#### 时序响应

逐时累计现金毛收益和逐时 ENS 用于补充说明边界场景在事件窗口内的动态响应。
